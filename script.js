...
// @resource     stylesheet https://raw.githubusercontent.com/WeArtStudioNL/Cake-s-Autofiller/master/style.css
    ...
    $(document).on("click", "#showprofits", function () {
        ...
        async function load() {
            ...
            for (var offset = 0; offset < 24; offset += 4) {
                ...
                $.getJSON("https://api.torn.com/user?selections=log&from=" + fromtime + "&to=" + totime + "&key=" + c_api_key, function (logdata) {
                    for (var main in logdata) {
                        if (main != undefined) {
                            for (var line in logdata[main]) {
                                if (line != undefined) {
                                    logs[new Date(logdata[main][line].timestamp * 1000).toLocaleString()] = logdata[main][line];
                                    currentkey++;
                                }
                            }
                        }
...
$("body").append(`
  <div id='c-autofiller-container'>
    <div id='ca-inner' style='position:relative;'>
        ...
        <input class='ca-inputtext' id='c_api_key' type='text' value='` + (c_api_key != undefined ? c_api_key : "Fill in your api key") + `'/>
      </span>
...
$(document).on("focus", "input.input-money", function () {
    ...
    if (ca_url.indexOf("add") >= 0) {
        ...
        itemname = $(this).parent().parent().parent().parent().prev().find(".image-wrap").find("img").attr("alt");
        itemid = imagesource.replace(/\//g, "");
        ...
        $.getJSON("https://api.torn.com/torn/" + itemid + "?selections=items&key=" + c_api_key, function (data) {
            ...
            $.getJSON("https://api.torn.com/market/" + itemid + "?selections=bazaar&key=" + c_api_key, function (databazaar) {
                ...
                if (checker[4] === undefined) {
                    ...
                    newchecker = checker[4].replace("</span", "");
                }
    ...
    if (ca_url.indexOf("manage") >= 0) {
        ...
        itemname = $(this).parent().parent().prev().prev().prev().prev().prev().children().attr("alt");
        itemid = imagesource.replace(/\//g, "");
...
$(document).on("click", ".checkMarketPrices", function () {
    ...
    getMoneyOnHand().then(money => {
        moneyonhand = money.money_onhand;
    });
